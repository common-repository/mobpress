...
class WPSDK_Introspector {
  ...
  public function get_current_post() {
    global $wpsdk_api;
    extract($wpsdk_api->query->get(array('id', 'slug', 'post_id', 'post_slug')));
    if ($id || $post_id) {
      if (!$id) {
        $id = $post_id;
      }
  ...
  public function get_current_category() {
    global $wpsdk_api;
    extract($wpsdk_api->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $wpsdk_api;
    extract($wpsdk_api->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $wpsdk_api;
    extract($wpsdk_api->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  public function get_comments($post_id) {
    ...
    foreach ($wp_comments as $wp_comment) {
      $comments[] = new WPSDK_Comment($wp_comment);
    }
    return $comments;
  }
  ...
  public function get_attachments($post_id) {
    ...
    if (!empty($wp_attachments)) {
      foreach ($wp_attachments as $wp_attachment) {
        $attachments[] = new WPSDK_Attachment($wp_attachment);
      }
    }
    return $attachments;
  }
  ...
  public function get_attachment($attachment_id) {
    ...
    return new WPSDK_Attachment($wp_attachment);
  }
  ...
  public function attach_child_posts(&$post) {
    ...
    foreach ($wp_children as $wp_post) {
      $new_post = new WPSDK_Post($wp_post);
  ...
  protected function get_category_object($wp_category) {
    ...
    return new WPSDK_Category($wp_category);
  }
  ...
  protected function set_posts_query($query = false) {
    ...
    if ($wpsdk_api->query->page) {
      $query['paged'] = $wpsdk_api->query->page;
    }
    
    if ($wpsdk_api->query->count) {
      $query['posts_per_page'] = $wpsdk_api->query->count;
    }
    
    if ($wpsdk_api->query->post_type) {
      $query['post_type'] = $wpsdk_api->query->post_type;
    }
    
    $query = apply_filters('wpsdk_query_args', $query);
    if (!empty($query)) {
      query_posts($query);
      do_action('wpsdk_query', $wp_query);
    }
  }
