...
class WPSDK_Response {
  
  function setup() {
    global $wpsdk_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($wpsdk_api->query->include) {
      $this->include_values = explode(',', $wpsdk_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($wpsdk_api->query->exclude) {
      $this->exclude_values = explode(',', $wpsdk_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    $data = apply_filters('wpsdk_encode', $data);
    ...
    if (function_exists('json_encode')) {
      ...
      if (version_compare(PHP_VERSION, '5.3') < 0) {
        ...
        if ($wpsdk_api->query->json_encode_options) {
          $json_encode_options = $wpsdk_api->query->json_encode_options;
        }
    ...
    if ($wpsdk_api->query->json_unescaped_unicode) {
  ...
  function respond($result, $status = 'ok', $http_status = 200) {
    ...
    if ($wpsdk_api->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($wpsdk_api->query->redirect) {
      $url = $this->add_status_query_var($wpsdk_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($wpsdk_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($wpsdk_api->query->callback, $json);
    } else {
      // Output the result
  ...
  function output($result, $http_status = 200) {
    $http_status = apply_filters('wpsdk_http_status', $http_status);
