...
class WPSDK_Comment {
  ...
  function WPSDK_Comment($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    if (!empty($wp_comment->user_id)) {
      $this->author = new WPSDK_Author($wp_comment->user_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function comment_id_not_found() {
    global $wpsdk_api;
    $wpsdk_api->error("Post ID '{$_REQUEST['post_id']}' not found.");
  }
  
  function comment_closed() {
    global $wpsdk_api;
    $wpsdk_api->error("Post is closed for comments.", 403);
  }
  
  function comment_on_draft() {
    global $wpsdk_api;
    $wpsdk_api->error("You cannot comment on unpublished posts.", 403);
  }
  
  function comment_post_redirect() {
    global $comment, $wpsdk_api;
    $status = ($comment->comment_approved) ? 'ok' : 'pending';
    $new_comment = new WPSDK_Comment($comment);
    $wpsdk_api->response->respond($new_comment, $status);
  }
