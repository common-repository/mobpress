...
class WPSDK_Author {
  ...
  // Note:
  //   WPSDK_Author objects can include additional values by using the
  //   author_meta query var.
  
  function WPSDK_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    global $wpsdk_api;
    if (!$wpsdk_api->query->author_meta) {
      return;
    }
    ...
    $vars = explode(',', $wpsdk_api->query->author_meta);
