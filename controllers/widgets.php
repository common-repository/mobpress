...
class WPSDK_Widgets_Controller {
  
  function get_sidebar() {
    global $wpsdk_api;
    $index = @$_REQUEST['sidebar_id'];
    if (empty($_REQUEST['sidebar_id'])) {
      $wpsdk_api->error("No sidebar specified. Include 'sidebar_id' var in your request.");
    } else if (!is_active_sidebar($index)) {
      $wpsdk_api->error("Sidebar '$index' is not active.");
    }
    ...
    $wpsdk_api_params = array();
    foreach ($widget_params as $param) {
      if (isset($_REQUEST[$param])) {
        $wpsdk_api_params[$param] = $_REQUEST[$param];
      }
    }
    ...
    foreach ( (array) $sidebars_widgets[$index] as $id ) {
      ...
      $params = array_merge(
        array( array_merge( $sidebar, array('widget_id' => $id, 'widget_name' => $wp_registered_widgets[$id]['name']), $wpsdk_api_params ) ),
